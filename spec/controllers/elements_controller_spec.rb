...
RSpec.describe ElementsController, type: :controller do
    ...
    describe 'PUT #update' do
        let(:current_element) { current_post.elements.build(valid_element_params.merge({ user: current_user, post: current_post  })) }
        before(:each) do
        ...
        it 'redirects admin to another page response' do
            ...
            put :update, params: { id: current_element.id, user_id: current_user.id, post_id: current_post.id, element: valid_element_params }
            expect(response.code).to eq('302')
        end
